...
- After engine-side IME composition changes, notify the active `TextEditor` through `ForceEditingChangedEvent()`. Keep the `TextEditor` from `InputInterface.ShowKeyboard`'s `requestee`; do not depend on `EditCoroutine` local-variable IL patterns to trigger `EditingChanged`.
