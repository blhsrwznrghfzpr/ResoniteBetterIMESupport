...
- IME IPC should set its own queue prefix through `Shared/ImeInterprocessQueue.cs` and pass the resulting queue name to InterprocessLib's custom `Messenger(ownerId, isAuthority, queueName)` constructor. Prefer `-shmprefix`, then a hash of `--bepinex-target`, then fallback.
